...
function setupMainer(msg: Message): void {
  ...
  msg.channel.send('React til denne besked med dem du mainer. Du kan godt vælge flere mains. Hvis du er i tvivl om, hvad emojies\'ne viser, kan du mouse-over. :thumbsup:')
    .then((message) => {
      const emojis = guild.emojis.cache.filter((emoji) => agents.includes(emoji.name));
...
function clearMessages(msg: Message, args: string[]): void {
  msg.channel.messages.fetch({ limit: parseInt(args[1], 10) })
...
function checkBitRate(msg: Message): void {
  ...
  guild.channels.cache.each((channel) => {
    if (channel.type === 'voice') {
      const warning: boolean = ((<VoiceChannel>channel).bitrate !== 64000);
      if (warning) output += '**';
...
function getDisplayNameFromID(msg: Message, args: string[]): void {
  const member = msg.guild?.members.resolve(args[1]);
  ...
  member.fetch().then((fetchedMember) => {
    msg.channel.send(`Display Name: ${fetchedMember.displayName}`);
  });
}

async function updateMainMessage(message: Message): Promise<Message | null> {
  const { guild } = message;
...
function admin(msg: Message, args: string[]): void {
  ...
  if (checkForDeveloper(member)) {
    ...
    if (args[0] === 'syncdata') JSONFileHandler.updateGuildData(guild);
